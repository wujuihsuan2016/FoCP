...
%\IEEEpubid{\makebox[\columnwidth]{\emph{[Long version of the LICS 2021 paper, with proofs in the appendix.]}\hfill}
...
%  \\[.5ex]
  ...
  \end{array}
\end{equation*}
Note that $\dual{\dual a} = a$.
We write $A\implies B$ as abbreviation for $\dual A\vlor B$.
...
modulo bound variable renaming, the
...
has a \bfit{label} $\labelof v\in L$ associated with it.
A graph $\gG$ is (partially) \bfit{coloured} if it carries a partial
equivalence relation $\linkingof\gG$ on $\vG$; each equivalence class
is a \bfit{colour}.\footnote{In \cite{hughes:fopws}
adjacent vertices are required to have distinct colours, since it
is part of the standard definition of colouring in graph
theory.
However, this condition is redundant (ommitted here), since it is implied
by the preclusion of bimatchings in the next section.}
...
\begin{definition}[{\cite[\S5]{hughes:fopws}}]
...
\begin{proof}
  ...
  none of the other rules has a premise of the form $\Scons{\exists
    ...
    \fequ}$, which is straightforward. For the $\fequ$-instances that
  do not involve the quantifiers, the details can be found
  in~\cite{dissvonlutz}, and the $\fequ$-instances concerning the
  quantifiers are admissible in the system without the $\exists$-rule.
\end{proof}

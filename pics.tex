...
\newcommand\fibheight{1.2}
\newcommand\peircefibheight{1.7}
\newcommand\fibheighttall{1.5}
...
\newcommand\veedrinkerformula{\exists x.(\cneg{p}x \vlor (\forall y.py))}
\newcommand\veedrinkerformulaOLD{\exists x\mkern1mu(\mkern1mu\ppx\mkern-2mu\vee\mkern-1mu \aypy)}
...
\begin{scope}[shift={(0,\fibheighttall)}]
...
\newcommand\drinkerfibcolouredpic{
  \begin{tikzpicture}
    \drinkerfibcoloured
  \end{tikzpicture}
}
...
\newcommand\cponformula[3]{% #1 cp, #2 xadjustment, #3 formula
  \begin{scope}[graph,shift={(#2,1.3)}]{#1}\end{scope}
...
\newcommand\peirceformula{((\cneg{p} \vlor q) \vlan \cneg{p}) \vlor p}
\newcommand\peirceformulaOLD{\left(\rule{0ex}{1.7ex}\mkern-1mu
(\pp\mkern-1mu\tightvee\mkern-3mu q)
\mkern-2mu\tightwedge\mkern-2mu
\pp\right)\mkern-1mu\tightvee\mkern-0mu p}

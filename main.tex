...
\title{Combinatorial Proofs and Decomposition Theorem for First-order Logic}
...
rectified form. A formula is \bfit{semi-rectified} if it can be rectified in a
unique way via variable renaming.
...
the sequent or formula obtained by replacing the hole in $S\{ \ \}$ with the formula $A$. Formally, 
  ...
  \[C ::= \Box \ | \ A \cor C \ | \ C \cand A \ | \ \exists x C \ | \ \forall x C \].
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    {\bf Figure 7.} A schema showing that the two branches of the same $\cor$ cannot be used in the cycle at the same time. 
    \end{center}

...
every literal is either $\ttt$-labelled or in a link. Hence, in a linked fograph no vertex is labeled~$\fff$.
...
\begin{thm}\label{thm:decomposition}
  For every derivation $\vlder{\FOKS}{\Deri}{A}{\ttt}$ there are $\fff$-free formulas $A_1,\ldots,A_5$, such that there is a derivation:
  \begin{equation*}
    \scalebox{.9}{$
    \vlderivation{
...
\begin{thm}\label{thm:decompositionA}
  For every derivation $\upsmash{\vlder{\FOKS}{\Deri}{A}{\ttt}}$ there is a formula~$A_1$ not containing any occurrence of $\fff$, such that there is a derivation:
  \begin{equation*}
    %\scalebox{.9}{$
    \vlderivation{
...
to the right-most combinatorial proof in Figures~\ref{fig:cps}
...
\begin{proof}
  First, observe that the $\exists$ rule can be permuted downwards
  over all the other rules since $A\ssubst{x}{t}$ has the same
...
\begin{proof}[Proof of Theorem~\ref{thm:MLS1->fonet}]
  ...
  $\vlan$ and $\vlor$.} \vadjust{\vskip-2ex}
  \begin{equation*}
    \scalebox{.95}{$
  ...
  \begin{equation*}
    \vlder{\set{\sw,\fequ}}{\DDeri_3}{ S_0\cons{S_1\cons{\cdots
          S_{k_l-1}\cons{S_{k_l}\cons{\dual q_{i,k_1}\vlan\dual
              q_{i,k_2}\vlan\cdots q_{i,k_l}\vlan B'}}\cdots}}}{
      S_0\cons{\dual q_{i,k_1}\vlan S_1\cons{\dual
          q_{i,k_2}\vlan\cdots S_{k_l-1}\cons{\dual q_{i,k_k}\vlan
            S_{k_l}\cons{B'}}\cdots}}}
  \end{equation*}
  where $k_1,\ldots,k_l$ are the indices of of the universal binders
...
\begin{proof}[Proof of Lemma~\ref{lem:cw->skew}]
  ...
  a existential preserving graph homomorphism $\graphof Q\to\graphof
  P$ and a fibration on the corresponding binding graphs.  Therefore,
  their composition $\mapof\Deri$ has the same properties fibration.
...
\begin{proof}
   ...
   a rule instance $\rr_1$ depends on an instance $\rr_2$, then
   $\rr_2$ occurs below $\rr_1$ in $\DDeri$.
   We can now lift $\DDeri$ stepwise. First, observe that each line of
   in $\DDeri$ is $\PE\fequ$-equivalent to the propositional encoding $\PE P$ of a
   first-order formula $P$.  We now have to show
   that each other instance of $\wrD$ in $\DDeri$ is indeed the image of a correct
...
Nonetheless, we hope to get new insights in the normalization of
...
two non-$\fequ$ rule instances seperated by $\fequ$. Only non-trivial cases are
presented here:
\begin{itemize}

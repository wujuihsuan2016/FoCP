...
x.(\cneg{p}(x) \vlor (\forall y.p(y)))}}{
            \vlin{\vlor}{}{\sqn{\cneg{p}(w), p(z), \cneg{p}(z) \vlor (\forall y.p(y)))}}{
              \vlin{\weakr}{}{\sqn{\cneg{p}(w), p(z), \cneg{p}(z), (\forall y.p(y)))}}{
                \vlin{\weakr}{}{\sqn{\cneg{p}(w), p(z), \ceng{p}(z)}}{
...
completeness is not \emph{full}.\todo{cite something for full comp?}
...
The fograph of drinker formula $\drinkerformula=\veedrinkerformula$:
%
...
Both $\veedrinkerformula$ and $\variantveedrinkerformula$ have the same
rectified fograph $\drinkergraph$, shown below-left.
%
...
\figdrinkerbifib Fig.\,\ref{fig:drinkerbifib} shows the drinker bifibration, binding fibration, and skeleton.

\leapfig Fig.\,\ref{fig:leap} shows a fonet with a unique dualizer, and its leap graph.
...
\end{array}
\end{equation*}
where the arity of $f$ and $p$ is $n$. We consider the truth constants
...
\bfit{corresponding formula} \todo{Case $n=0$!} of a sequent $\Gamma=A_1,A_2,\ldots,A_n$

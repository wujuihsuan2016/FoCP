...
rectified. If $A = (\forall x. p(x)) \vlor (\forall x. q(x))$, then $\graphof A = \single x \quad
\single p(x) \quad \single x \quad \single q(x)$, the scope of each $x$-binder
contains all the vertices, in particular, the two $x$-binders.}

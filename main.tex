...
\newcommand\acDeq {\mathsf{ac}^\fequ}
...
\begin{abstract}
  ...
  decomposition theorem that establishes a certain kind of normal form
...
\begin{proof}
  ...
  \vlderivation{
    \vlin{\cD}{}{(A \vlor B) \vlan (C \vlor D)}{
      \vlin{\wlD}{}{((A \vlor B) \vlan (C \vlor D)) \vlor ((A \vlor B) \vlan (C \vlor D))}{
	\vlin{\wlD}{}{((A \vlor B) \vlan (C \vlor D)) \vlor ((A \vlor B) \vlan D))}{
  ...
  \vlderivation{
    \vlin{\cD}{}{\exists x.(A \vlor B)}{
      \vlin{\wlD}{}{(\exists x.(A \vlor B)) \vlor (\exists x.(A \vlor B))}{
  ...
  \vlderivation{
    \vlin{\cD}{}{\forall x.(A \vlor B)}{
      \vlin{\wlD}{}{(\forall x.(A \vlor B)) \vlor (\forall x.(A \vlor B))}{
...
In this section we establish the relation between ske bifibrations and derivations  in
...
\bfit{recification} $\rectif\Deri$ by rectifying each line of $\Deri$, yielding a derivation in 
...
\begin{lemma}\label{skew->cw}
  Let $\gA$ and $\gB$ be fographs, let $\phi\colon\gA\to\gB$ be a skew bifibration, and let $A$ and $B$
  be formulas with $\graphof A=\gA$ and $\graphof B=\gB$. Then there
  are derivations
  \begin{equation*}
    \vlderivation{
...
\begin{proof}
  ...
  $\vB'$. Hence, we have two maps
  $\phi''\colon\gA\to\gB_1$ being a surjection and $\phi'\colon
  \gB_1\to\gB$ being an injection that reflects edges. Both, $\phi'$
  and $\phi''$ remain skew bifibrations. Let us first look at $\phi'$.
  Let $\PE B_1$ be the propositional formula obtained from
  $\PE B$ by removing all atoms that are not represented by vertices
  in $\vB'$. Then $\graphof{\PE B_1}=\gB_1$. 
  By \cite[Proposition~7.6.1]{str:07:RTA}, we have a derivation
  $\vlder{\set{\wrD,\fequ}}{\PE\Deri_1}{\PE B}{\PE B_1}$.  A
  subformula of $\PE B$ is called \emph{weak} if it has been
  ...
  collapsing several weakenings into a single one.}  Now observe that
  whenever a weakening in $\PE\Deri_1$ deletes an atom $x\in\VAR$, it
  must also delete all atoms in the scope of the corresponding
  quantifier, because $\phi$ is a fibration on the binding
  graph. Hence, each line in $\PE\Deri_1$ is the propositional
  encoding $\PE P$ of a first-order formula $P$. Thus we have a first
  order derivation $\vlder{\set{\wrD,\fequ}}{\Deri_1}{B}{B_1}$. we now
  have to show that each instance of $\wlD$ or $\wrD$ is indeed a
  correct application in first-order logic. For this we have to
  inspect the cases a weakening happens inside a subformula $x\vlor C$
  or $x\vlan C$ in $\PE\Deri_1$. There are the following cases:
  ...
  preserve existentials, this third case cannot occur, and therefore
  $\Deri_1$ is a valid first-order derivation in $\set{\wD,\fequ}$.
  ...
  where $\PE A_1 =\PE A\substof\phi$ is the result of applying $\substof\phi$
  to~$\PE A$. Note that $\PE A_1 = \PE{(A\substof\phi)}$. To $\Deri_2$ we can apply a permutation argument,
  permuting all instances of $\acD$ up until they either reach the top
  of the derivation or an instance of $\me$ which separates the two
  atoms in the premise. More precisely, we consider the following
  inferene rule
  \begin{equation}
    \vlinf{\acDeq}{}{\Scons{a}}{S_0\cons{S_1\cons{a}\vlor S_2\cons{a}}}
  \end{equation}
  where $S_1\conhole\fequ\conhole\vlor E$ and
  $S_2\conhole\fequ\conhole\vlor F$ and $S\conhole\fequ
  S_0\cons{\conhole\vlor E\vlor F}$ for some formulas $E$ and $F$. The
  rule $\acDeq$ permutes over $\fequ$ and other instances of $\acDeq$,
  ...
  \begin{equation}
    \label{eq:mac}
    \vlderivation{
      \vlin{\acDeq}{}{\Scons{R\cons{a}\vlan(C\vlor D)}}{
        \vlin{\me}{}{\Scons{(R_1\cons{a}\vlor R_2\cons{a})\vlan(C\vlor D)}}{
          \vlhy{\Scons{(R_1\cons{a}\vlan C)\vlor (R_2\cons{a}\vlor D)}}}}}
  \end{equation}
  Now we translate $\PE\Deri_2$ into a first-order derivation
  $\vlder{\set{\acD,\me,\mfaD,\mexD,\fequ}}{\Deri_2}{B_1}{A\substof\phi}$
  by ``decoding'' every line in $\PE\Deri_2$. I.e., for each line $Q$
  in $\PE\Deri_2$ we construct a formula $P$ with $\PE P\fequ Q$.  We
  proceed by induction on the height of $\PE\Deri_2$, beginning at the
  bottom. We already constructed the formula~$B_1$. Now consider the bottommost rule in $\PE\Deri_2$. If it is a $\fequ$, then we trivially can go to the next step. If it is a $\acD$ duplicating an atom that is not a $x\in\VAR$, then this $\acD$ is also a valid inference step in the first-order system. Similarly, any instance of $\me$ that is not in a situation as in~\eqref{eq:mac} can be i
\end{proof}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

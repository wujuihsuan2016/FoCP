...
\fff$, and $\overline{\fff} = \ttt$. \bfit{Formulas} $f, g, h$ are generated inductively from atoms by binary
connectives $\vee$ and $\wedge$ and quantifiers $\forall x$ and $\exists x$.
The \bfit{negation} $\neg$ is defined in the usual way.
...
$N(v)$ of $v \in V$ is defined as the set $\{w | vw \in E\}$, and a module is a

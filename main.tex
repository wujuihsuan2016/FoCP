...
$f$ (resp. $p$) is an $n$-ary function (resp. predicate) symbol and $t_1, \ldots,
t_n$ are terms, then $ft_1 \ldots t_n$ (resp. $pt_1 \ldots t_n$) is a term
...
\subseteq$ or $M \cap M' = \emptyset$. A \bfit{graph homomorphism} $h: (V, E)
\rightarrow (V', E')$ is a function $h : V \rightarrow V'$ such that $h(v)h(w)
\in E'$ whenever $vw \in E$.

\subsection{First-order Graphs}
...
A graph homomorphism $h: (V, E) \rightarrow (V', E')$ is a \bfit{fibration} if
for all $v \in V$ and $wh(v) \in E'$, there exists a unique $\tilde{w}$ such
that $\tilde{w}v \in E$ and $h(\tilde{w}) = w$, and is a \bfit{skew fibration}
if for all $v \in V$ and $wh(v) \in E'$ there exists $\tilde{w}$ such that
$\tilde{w}v \in E$ and $h(\tilde{w})w \notin E'$. A \bfit{fograph homomorphism}
is a graph homomorphism between the underlying graphs of the fographs
considered. A fograph homomorphism $h : G \rightarrow G'$ is
\bfit{label-preserving} if forall $v \in V_G, label(v) = label(h(v))$, and is
\bfit{existential-preserving} if for all existental binder $b$ in $G$, the
vertex $h(b)$ is an existential binder in $G'$.

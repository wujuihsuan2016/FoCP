...
\begin{definition}
A \bfit{first-order combinatorial proof} (\bfit{FOCP}) of a fograph $G$ is a skew
...
\begin{thm}\label{thm:CP-DI}
  ...
  there is a combinatiorial proof $\phi\colon\gC\to\graphof A$ such that $\gC=\graphof{\rectif{A'}}$.
\end{thm}

Furthermore, in the proof of Theorem~\ref{thm:CP-DI}, we will see that
(i) the links in the fonet $\gC$ correspond precicely to the pairs of
atoms that meet in the instances of the $\aiD$-rule in the derivation
$\Deri_1$, and (ii) the ''flow-graph'' of $\Deri_2$ that traces the
quatifier- and atom-occurrences in the derivation corresponds exactly
...
combinatorial proofs form a canononical proof representation for first
...
have here and alterantive proof of the soundness and competeness of

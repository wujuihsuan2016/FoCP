...
\begin{itemize}
  ...
  \item $\Gr(A^P) = \Gr(A)$ by confounding the atoms $U_x$, $E_x$ with the variable
	  $x$. Thus, a map $f : \Gr(A^P) \rightarrow \Gr(B^P)$ can be seen as a map
...
\begin{center}
for each $x$-binder $b$ in $\Gr(B^P)$, for each vertex $v \in V(\Gr(A^P))$

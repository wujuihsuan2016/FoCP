...
\lutz{actually NO! we can prove $\sqn{\fff,\ttt}$ using $\ax$ without a special rule for $\fff$. So can you please undo the changes and delete the comments?}
\begin{proof}
  This is a straightforward exercise that we leave to the reader. (Note that the $\ax$-rule can be applied to $\sqn{\fff,\ttt}$ in the cases of $\rr \in \set{\aiD, \faD}$.)
\end{proof}
...
First, if an instance $\vlinf{\weakr}{}{\sqn{\Gamma,A}}{\sqn{\Gamma}}$
 of $\weakr$ is followed by a rule in which $A$ is not in the principal
...
\vlderivation{
  \vlde{\set{\weakr, \wrD,\cD,\fequ}}{\Deri'_2}{\sqns{\Gamma}}{
    \vltrl{}{\FOMLL}{\Deri'_1}{\sqns{\Gamma_0}}{
      ...
      \vlhy{}}}}
\end{equation*}
Note that $\fequ$ is required here since the permutaion of formulas is implicit
in $\FOMLL$.
...
define its \bfit{rectification} $\rectif\Deri$ inductively, by recifying each
...
$\dsubstof\Phi$ and call it the \bfit{dualizer} of $\Deri$. Furthermore, every atom occuring in the conclusion $A$
...
is to construct a proposional linked cograph, called the \bfit{frame}~\cite{hughes:unifn}
...
step~1 above.  Now we define the \bfit{frame} of $\gC$, denoted
$\frameof\gC$, as
$\tuple{\graphof{\frameof\Gamma},\linkingof{\frameof\Gamma}}$.

\begin{lemma}\label{lem:frame}
  A linked fograph $\gC$ induces a bimatching iff its frame $\frameof\gC$ induces a bimatching.
\end{lemma}

Note that the linking $\lambda_m$ is a valid $\MLLm$ proof net.


We define the \bfit{frame} of a linked fograph to be the linked cograph defined as follows:
...
\begin{lemma}
  ...
  $\vlderivation{
    \vlde{\set{\wrD,\rectif\acD,\me,\rectif\mfaD,\rectif\mexD,\fequ}}{\rectif\Deri}{\rectif
...
into an instance of the same rule. It's trivial but may be worth mentioning.}\lutz{done below}
  ...
  variabes---the propositional variable, as well as the first-order
...
\begin{proof}[Proof of Lemma~\ref{skew->cw}]
  ...
  the premise. More precisely, we consider the following inferene rule
  \begin{equation}
    \label{eq:acDeq}

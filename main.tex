...
\begin{proof}[Proof of Lemma~\ref{lem:cw->skew}]
  ...
  $\rr\in\set{\wrD,\acD,\me,\rectif\mfaD,\rectif\mexD,\fequ}$
...
into an instance of the same rule. It's trivial but may be worth mentioning.}
  ...
  variables---the atom variable
  \juihsuan{or propositional variable? maybe use just a single name}, as well as the first-order variable of
  the same name---as everything is rectified, there is no ambiguity
  here.
...
\bfit{lifted} to $\sysS'$ if there is a formula $C$ such that
...
\begin{proof}[Proof of Lemma~\ref{skew->cw}]
  ...
  reflects edges. \juihsuan{what do you mean by "reflect edges"?} Both, $\phi'$ and $\phi''$ remain skew
  ...
  \juihsuan{$B_1$ is not defined (The notation $\PE B_1$ suggests that it is the
propositional encoding of some formula but we need to prove it). This can be
proven using the fact that $\phi'$ is a skew bifibration here.}
  ...
  permuting all instances of $\acD$, that duplicate an atom $x\in\VAR$, up until they either reach the top
  ...
  \end{equation}
  Now we show that $\PE\Deri_2$ can be lifted to
  $\set{\acD,\me,\mfaD,\mexD,\fequ}$.  We proceed by induction on the
  height of $\PE\Deri_2$, beginning at the bottom, as we already
  constructed the formula~$B_1$.  Every instance of $\fequ$ or $\acD$
  (duplicating an atom that is not a $x\in\VAR$), can trivially be
  lifted. Similarly, an instance of $\me$, that is above an instance
  of $\acDeq$ as indicated in~\eqref{eq:mac}, can be lifted as the
  premise must also be a propositional encoding and the $\me$-inference
  remains correct in the first-order case.  It remains to consider a
  situation~\eqref{eq:mac}. Observe that we must have
  ...
  \item $E$ is non-empty and $F$ is empty: This case is similar to the
    previous one.
  \end{itemize}
  Finally, we need to consider the instances of $\acDeq$ that are at the top of the derivation.
\end{proof}

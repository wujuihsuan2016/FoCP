...
the set $\set{w \mid vw \in\eG}$. A \bfit{module} is a set $M \subseteq \vG$ such that $N(v) \ M = N(w) \ M$
...
Figure~\ref{fig:KS1} shows the inference rule for the deep inference
system~$\FOKS$ that we are using this paper. It is a slight variation
...
that we (i) the explicit presence of the $\mixr$-rule, (ii) a
...
elsewhere~\cite{brunnler:,alertubella:}.\footnote{In the deep
...
We are now ready to see the main results of this paper.

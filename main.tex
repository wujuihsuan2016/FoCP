...
the $\mixr$-rule. The linear fragment of $\FOLK$, i.e., the fragment
...
\times \gH$ is the graph $\tuple{\vG \cup\vH,\eG \cup \eH \cup \set{vw
    | v \in \vG, w \in \vH}}$.  A graph $\gG$ is \bfit{disconnected}
...
A graph $\gG$ is \bfit{labelled} in a set $L$ if
each vertex $v\in\vG$ has an element $\ell(v)\in L$ associated with it, its \bfit{label}.

A \bfit{directed graph} $\gG =\tuple{\vG,\eG}$ is set $V$ of
\bfit{vertices} and a set $\eG\subseteq \vG\times\vG$ of \bfit{direct edges}.
A \bfit{directed graph homomorphism}
$\phi\colon\tuple{\vG,\eG}\to\tuple{\vH,\eH}$ is function $\phi\colon\vG\to\vH$ such that if

...
\begin{lemma}
  \label{lem:me}
  The rules $\me,\mfaD,\mexD$ are derivable in $\set{\wlD,\wrD,\cD}$. 
\end{lemma}
...
$\set{\wlD,\wrD,\acD,\me,\mfaD,\mexD,\fequ}$. However, 
...
$\set{\wlD,\wrD,\acD,\me,\rectif\mfaD,\rectif\mexD,\fequ}$ we can
...
derivation $\Deri$ in
$\set{\wlD,\wrD,\acD,\me,\rectif\mfaD,\rectif\mexD,\fequ}$ we can then define
...
\begin{proof}[Proof of Lemma~\ref{lem:cw->skew}]
  For $\Deri$, we construct its propositional encoding $\PE\Deri$ by
  translating every line into its proposional encoding. The instances of the rules
  $\rectif\mfaD$ and $\rectif\mexD$ are replaced two steps by:
  \begin{equation*}
    \vlderivation{
  ...
  This gives us a derivation $\vlderivation{
    \vlde{\set{\wlD,\wrD,\acD,\rectif\acD,\me,\PE\fequ}}{\PE\Deri}{\PE
  ...
  $\mapof\Deri$ is a skew fibration. \todo{show the other properties,
    once they are fixed}
\end{proof}
\subsection{From Skew Bifibrations to Contraction and Weakening}

\begin{lemma}\label{skew->cw}
  Let $\phi\colon\gA\to\gB$ be a skew bifibration, and let $A$ and $B$
  be formulas with $\graphof A=\gA$ and $\graphof B=\gB$. Then there
  is a derivation $\vlderivation{
    \vlde{\set{\wlD,\wrD,\acD,\me,\rectif\mfaD,\rectif\mexD,\fequ}}{\Deri}{B}{
...
\begin{proof}
  ...
  $\vB'$. Now let $\PE B_1$ be the propositional formula obtained from
  $\PE B$ by removing all atoms that are not represented by vertices
  in $\vB'$. Then $\graphof{\PE B_1}=\gB_1$. Furthermore, the map
  $\phi'\colon \gB_1\to\gB$ is an injection that reflects
  edges. Hence, by \cite[Proposition~7.6.1]{str:07:RTA}, we
  have a derivation
  $\vlder{\set{\wlD,\wrD}}{\PE\Deri_1}{\PE B}{\PE B_1}$. \todo{show that we have $B_1$ with $\graphof{B_1}=\gB_1$, so that we can can get
  first order derivation $\vlder{\set{\wlD,\wrD}}{\Deri_1}{B}{B_1}$.}

  Now, let $\sigma$ be the substitution induced by $\phi$, and let
  $\gA_1=\gA\sigma$ be the graph obtained from $\gA$ by applying
  $\sigma$ to all the labels. Note that $\gA_1$ is not neccesarily a
  fograph, as binders might be illegal. But it still is a cograph, and
  we have a surjective skew fibration $\phi''\colon\gA_1\to\gB_1$ that
  preserves the labels. Therefore, by
  \cite[Proposition~7.5]{str:ral:tableaux19}, there is a derivation
  $\vlder{\set{\acD,\me,\PE\fequ}}{\PE\Deri_2}{\PE B_1}{\PE A_1}$,
  where $\PE A_1 =\PE A\sigma$ is the result of applying $\sigma$
  to~$\PE A$. To $\Deri_2$ we can apply a permutation argument,
  ...
  atoms in the premise. More precisely, we consider the following inferene rule
  \begin{equation}

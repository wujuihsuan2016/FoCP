...
\begin{thm}
  A formula $A$ is a theorem of first-order logic iff the sequent $\sqn A$ is derivable in $\FOLK$.
\end{thm}
...
is \bfit{induced} if $v,w\in\vH$ and $vw\in\eG$ implies $vw\in\eH$.  A
graph is \bfit{$\gH$-free} if it does not contain $\gH$ as an induced
...
the set $\set{w \mid vw \in\eG}$. A set $W \subseteq \vG$ induces a \bfit{matching} in
$\gG$ if for all $w \in W$, $N(w) \cap W$ is a singleton.
A \bfit{module} is a set $M \subseteq \vG$ such that $N(v) \ M = N(w) \ M$
...
\begin{equation*}
  \begin{array}{l}
    \graphof{\alpha} = \cdot \alpha (\text{single-vertex graph whose vertex is 
    labelled by } \alpha); \\[.5ex]
    \graphof{A \cor B} = \graphof{A} + \graphof{B};
    \graphof{A \cand B} = \graphof{A} \times \graphof{B}; \\[.5ex]
    \graphof{\forall x . A} = \cdot x + \graphof{A};
    \graphof{\exists x . A} = \cdot x \times \graphof{A}.
  \end{array}
\end{equation*}
\todo{make look nice, use array} \juihsuan{done}\lutz{use the single macro (see above), so that we can change the notation later if we need to (and make the array with two columns)}
...
We define a congruence relation on formulas by the following equations:
\begin{equation*}
  \begin{array}{cc}
    A \vlan B \fequ B \vlan A & \forall x . (A \vlor B) \fequ (\forall x . A)
    \vlor B \\
    A \vlor B \fequ B \vlor A & \exists x . (A \vlan B) \fequ (\exists x . A)
    \vlan B \\
    (A \vlan B) \vlan C \fequ A \vlan (B \vlan C) & \forall x . \forall y . A
    \fequ \forall y . \forall x . A \\
    (A \vlor B) \vlor C \fequ A \vlor (B \vlor C) & \exists x . \exists y . A
    \fequ \exists y . \exists x . A \\
  \end{array}
\end{equation*}
...
graph $(\vG, \lpG)$ where $\lpG$ is the set of leaps of $\gG$. A set $W
\subseteq \vG$ induces a
\bfit{bimatching} in $\gG$ if $W$ induces a matching in $\gG$ and in the leap graph
$\lgG$ as well.
  ...
  \begin{center}
    \framebox{%
      ...
      \begin{array}{@{}c@{}}
        ...
        \\ \\[-1ex]
        \vlinf{\wD}{}{\Scons{A\vlor B}}{\Scons{A}}
        \qquad
...
rewriting rules inside any (positve) formula or sequent

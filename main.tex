...
counrable set $\PRED=\set{p,q,\ldots}$ of predicate symbols. Each function symbol and
...
\end{array}
\end{equation*}
where the arity of $t$ and $p$ is $n$. Note, that in this paper we
consider the truth constants $\ttt$ (\emph{true}) and $\fff$
(\emph{false}) as atoms, and we consider all formulas in negation
normalform. The \bfit{negation} $(\dual\cdot)$ is defined for all
...
\begin{equation*}
  ...
  \dual\fff = \ttt
  \quad
  \dual{p(t_1 \ldots t_n)} = \dual{p}(t_1 \ldots t_n)
  \quad
  \dual{\dual p(t_1 \ldots t_n)} = p(t_1 \ldots t_n)
\\
  \dual{\dual a} = a
\end{equation*}
...
all free variables. As every formula has a logically equivalent
rectified form, we will most of the results in this paper to rectified formulas.
...
We consider Gentzen's sequent calculus system $\LK$ \cite{Gentzen 1935}.
...
Consider the following deep inference rules \cite{BT 2001}:
\begin{center}
\mbox{
\begin{prooftree}

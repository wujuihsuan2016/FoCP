...
A graph homomorphism $f: \tuple{\vG, \eG} \rightarrow \tuple{\vH, \eH}$ is a \bfit{fibration} if
...
\begin{definition}
  Let $\gG$ and $\gH$ be forgraphs. A \bfit{skew bifibration} $\phi\colon\gG\to\gH$ is a fograph homomorphism that is a skew fibration on $\tuple{\vG, \eG} \to \tuple{\vH, \eH}$ and a fibration on the binding graphs $\bG\to\bH$.
...
In this paragraph, we associate to each formula $A$ its \bfit{formula tree}
...
unification problem, and a dualizer of the linking $\lambda$ is an assignment
...
A derivation $\Deri$ is \bfit{rectified} if every line in $\Deri$ is rectified.

\todo{define rectification $\rectif\Deri$ of a derivation  $~\vlder{\FOMLS}{\Deri}{A}{\ttt}$}
\todo{define substitution of a derivation $\substof{\Deri}$ and linking of a derivation $\linkingof{\Deri}$ }

\begin{thm}
  Let the derivation $\vlder{\FOMLS}{\Deri}{A}{\ttt}$ be given. Then
...
\begin{thm}
  Let $\gC$ be a fonet with linking $\linkingof\gC$ and dualizer
  $\delta$, and let $A$ be a formula with $\graphof A=\gC$.  Then
  there is a derivation $\vlder{\FOMLS}{\Deri}{A}{\ttt}$ such that
  $\linkingof\gC=\linkingof{\Deri}$ and $\delta=\substof{\Deri}$.
\end{thm}
...
\todo{define rules $\rectif\mfaD,\rectif\mexD$}

\todo{define $\mapof\Deri$ (underlying map doing the flowgraph thing)}

\begin{lemma}
  Let $\vlderivation{
    \vlde{\set{\wlD,\wrD,\acD,\me,\rectif\mfaD,\rectif\mexD,\fequ}}{\Deri}{B}{
      \vlhy{A}}}$ be a rectified derivation. Then the map

...
equivalence relation $\samcol$ on $\vG$; each equivalence class is a
...
colouring and/or labelling inherited (i.e., $\hat v\samcol\hat w$ if
...
$x$-literal in its scope.  In a logical cograph $G$, a binder $b$ is
\bfit{existential} (resp. \bfit{universal}) if, for every other vertex
$v$ in its scope, we have $bv \in E_G$ (resp. $bv \notin E_G$). An
...
associated fograph $\graphof(A)$ inductively by: $\graphof(\alpha) =
\cdot \alpha$ (a single-vertex graph whose vertex is labelled by
$\alpha$); $\graphof(A \cor B) = \graphof(A) + \graphof(B)$ and
$\graphof(A \cand B) = \graphof(A) \times \graphof(B)$.
\todo{make look nice, use array}
...
\begin{proof}
  ...
  \todo{}\lutz{maybe mention also fn 2-5 in arxiv-paper}
\end{proof}

...
\todo{this has to be moved to the right place at some point}

The \bfit{propositional encoding} $\PE{A}$ of a formula $A$ is defined inductively by:
\begin{equation*}
  \begin{array}{r@{\;=\;}l}
    \PE{a} & a\\
  ...
  \qquad
  \begin{array}{r@{\;=\;}l}
    \PE{(\forall x A)} & u_x \cor \PE{A}\\
    \PE{(\exists x A)} & e_x \cand \PE{A}
  \end{array}
\end{equation*}
where $u_x$ and $e_x$ are fresh nullary atoms. The purpose of the propositional encoding is to simplify some arguments that do not depend on the variable bindings. In particular, observe that $\graphof A$ and $\graphof{\PE A}$ only differ in the labelling of certain vertices.
...
quantifiers, and edges directed from leaves to the root.

Let $\Gamma$ be a sequent in $\FOMLL$. Consider the formula tree
$\formtree{\form{\Gamma}}$. A \bfit{link} on $\Gamma$ is a pair of
leaves whose atoms are pre-dual. A \bfit{linking} $\lambda$ on $\Gamma$ is a set
of
disjoint links such that each leaf of $\Gamma$ is either labelled by $\ttt$ or
...
derivation $\Deri$ in
$\set{\wlD,\wrD,\acD,\me,\rectif\mfaD,\rectif\mexD,\fequ}$ we define
...
\begin{proof}
  ...
  translating every line into its proposional encoding. The rules
  $\rectif\mfaD$ and $\rectif\mexD$ are replaced by:
  \begin{equation*}
    \vlderivation{
      \vlin{\rectif\acD}{}{\Scons{u_x\vlor\PE{(Ax)}\vlor\PE{(Ax)}}}{
        \vlin{\fequ}{}{\Scons{(u_y\vlor u_z)\vlor(\PE{(Ax)}\vlor\PE{(Ax)})}}{
          \vlhy{\Scons{(u_y\vlor\PE{(Ax)})\vlor (u_z\vlor\PE{(Ax)})}}}}}
  \end{equation*}
  and
  \begin{equation*}
    \vlderivation{
      \vlin{\rectif\acD}{}{\Scons{e_x\vlan\PE{(Ax)}\vlor\PE{(Ax)}}}{
        \vlin{\me}{}{\Scons{(e_y\vlor e_z)\vlan(\PE{(Ax)}\vlor\PE{(Ax)})}}{
          \vlhy{\Scons{(e_y\vlan\PE{(Ax)})\vlor (e_z\vlan\PE{(Ax)})}}}}}
  \end{equation*}
  respectively. This gives us a derivation $\vlderivation{
    \vlde{\set{\wlD,\wrD,\acD,\rectif\acD,\me,\fequ}}{\PE\Deri}{\PE
      B}{ \vlhy{\PE A}}}$ with $\mapof{\PE\Deri}$ being equal to
  $\mapof\Deri$, modulo relabeling of of the binders. It has been
...
\begin{proof}
  Let $\PE\gA=\graphof{\PE A}$ and $\PE\gB=\graphof{\PE B}$ which are equal to $\gA$ and $\gB$, respectively, modulo relabeling of the binders. Thus, we can define $\PE\phi\colon\PE\gA\to\PE\gB$, which is a skew fibration as well. 
\end{proof}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

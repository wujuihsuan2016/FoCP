...
underlying graphs, and $\substof\phi$ is a variable renaming such that
for all $v\in\vG$ we have
$\labelof{\phi(v)}=\substof\phi(\labelof v)$. Note that this entails that
$\phi$ maps binders to binders and literals to literals. We say that a
...
versa, as we might have multiple occurrences of bound varibles in $Q$,
such that $\graphof D$ is still a fograph.
...
\begin{proof}[Proof of Lemma~\ref{skew->cw}]
  ...
  always knowing that we can add instances of $\fequ$ as needed. \juihsuan{this
does not seem clear to me. What if from $\PE A_1$ to $\PE B_1$ there are just a
bunch of $\PE\fequ$? What do we do in this case?} To
  ...
  \item $\acDx$: We know that the premisse of~\eqref{eq:acDeq} is a
    ...
    \item $E$ and $F$ are both non-empty: We have
      ...
      \end{equation*}
    \juihsuan{We should say that $\Sconhole, E, $ and $F$ are also propositional
encodings. Here I think you kind of identify a formula with its prop. encoding.}

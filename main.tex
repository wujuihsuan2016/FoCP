...
\begin{tikzpicture}
  ...
  \draw (0, 0) node (par1) {$\vee$};
  \draw (1.8, 1.2) node (z) {\color{blue} $\forall z$};
  \draw (-1.8, 1.2) node (par2) {$\vee$};
...
\begin{tikzpicture}
  \tikzstyle{every node}=[circle, minimum size=20pt, inner sep=2pt]
  \draw (0, 0) node (par1) {$\vee$};
  \draw (1, 0.9) node (par3) {\color{blue} $\vee$};
  \draw (-1.8, 1.2) node (par2) {$\vee$};
  \draw (-2.6, 2.4) node (tens1) {\color{red} $\wedge$};
  \draw (-1, 2.4) node (tens2) {$\wedge$};
  ...
  \draw (1.6, 1.8) node (par4) {$\vee$};
  \draw (2.2, 2.7) node (tens3) {$\wedge$};
  \draw (2, 3.6) node (P) {$P$};
...
\mbox{
\begin{prooftree}
  \hypo{(E_x \wedge C) \vee (E_x \wedge D)}
  \infer1[$\me$]{E_x \wedge (C \vee D)}
\end{prooftree}

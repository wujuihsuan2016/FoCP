...
A \bfit{variable renaming} is a substitution $\sigma$ with $\sigma(x)\in\VAR$ for all variables $x$.
...
A \bfit{fograph homomorphism} $\phi=\tuple{\phi,\substof\phi}$ is a
pair where $\phi\colon\gG\to\gH$ is a graph homomorphism between the
underlying graphs, and $\substof\phi$, also called the \bfit{substitution induced by $\phi$} is a variable renaming such that
for all $v\in\vG$ we have
$\labelof{\phi(v)}=\substof\phi(\labelof v)$. Note that this entails that

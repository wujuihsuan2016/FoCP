...
Retor\'e~\cite{retore:03,retore:??}\lutz{that ``handsome'' report he mentioned in
  the talk}. We use it now for our sequentialization:
...
\begin{proof}
  ...
  $\exists x.A$ such that the correponding binder $\single x$ in $\gC$
  has no dependency, then we can apply the $\exists$-rule, chosing the

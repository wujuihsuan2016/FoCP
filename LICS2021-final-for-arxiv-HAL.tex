...
\bfit{dual}
$\dual{p}$. The grammars below generate
...
has an element $\labelof v\in L$ associated with it, its \bfit{label}.
...
is a \bfit{colour}.\footnote{In~\cite{hughes:fopws} it is also
required that adjacent vertices have different colours, because it
is part of the standard definition of colouring in graph
theory.
However, this condition is redundant (and therefore ommitted here), since it is implied
